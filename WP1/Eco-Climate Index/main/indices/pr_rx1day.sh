...
v=pr
vn=highest_one_day_precipitation_amount_per_time_period
idx=rx1day
...
set -e
CDO chname,$vn,$idx -eca_rx1day $ftm $fou
ncatted -O -a long_name,$idx,m,c,"highest_one_day_precipitation_amount" $fou
ncatted -O -a standard_name,$idx,m,c,"Highest one day Precipitation Amount" $fou
ncatted -O -a units,$idx,m,c,"mm/day" $fou
